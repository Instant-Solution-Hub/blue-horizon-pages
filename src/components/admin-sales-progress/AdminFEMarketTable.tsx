...
interface AdminFEMarketTableProps {
  data: FEMarketSale[];
}

const AdminFEMarketTable = ({ data }: AdminFEMarketTableProps) => {
  if (data.length === 0) {
    return (
      <Card className="border border-border shadow-sm">
        <CardHeader className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-primary/10 py-4">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-lg font-semibold text-foreground">Market Wise Sales (FE)</CardTitle>
          </div>
        </CardHeader>
          ...
          <TableBody>
            {data.map((row, index) => (
              <TableRow key={index} className="hover:bg-primary/5 transition-colors border-b border-primary/10">
                ...
                <TableCell className="font-semibold text-green-600">₹{row.secondarySales.toLocaleString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
...
export default AdminFEMarketTable;
