...
import { fetchCompanyTarget, updateCompanyTarget } from "@/services/SalesProgressService"; // adjust path
...
const CompanyTargetSection = () => {
  ...
  const percentage =
    companyTarget > 0
      ? Math.min((companyProgress / companyTarget) * 100, 100)
      : 0;

  // Fetch data on load
  const loadCompanyTarget = async () => {
    try {
      const data = await fetchCompanyTarget();
      if (data) {
        setCompanyTarget(data.targetAmount || 0);
        setCompanyProgress(data.currentProgress || 0);
      }
    } catch (err) {
      console.error("Error fetching company target", err);
    }
  };
  ...
  if (isNaN(newTarget) || isNaN(newProgress)) return;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-muted/50 rounded-lg p-4 text-center">
              ...
              <p className="text-xl font-bold text-primary">
                {formatCurrency(companyProgress)}
              </p>
            </div>
            ...
            <div className="bg-muted/50 rounded-lg p-4 text-center">
              ...
              <p className="text-xl font-bold text-destructive">
                {companyProgress >= companyTarget
                  ? "—"
                  : formatCurrency(companyTarget - companyProgress)}
              </p>
            </div>
          </div>
        )}
