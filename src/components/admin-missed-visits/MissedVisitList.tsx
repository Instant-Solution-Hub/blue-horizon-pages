...
import { fetchAllFEMissedVisits, fetchAllManagerMissedVisits, markFEVisitAsCompleted, markManagerVisitAsCompleted } from "@/services/AdminSlotService";
...
export function MissedVisitList() {
  ...
  const [feVisits, setFEVisits] = useState([]);
  const [managerVisits, setManagerVisits] = useState([]);
  const [selectedVisitId, setSelectedVisitId] = useState<string | null>(null);
  const [completionNotes, setCompletionNotes] = useState("");
  const { toast } = useToast();

  useEffect(() => {
    getAllFEMissedVisits();
    getAllManagerMissedVisits();
  }, [])

  const getAllFEMissedVisits = async () => {
    const response = await fetchAllFEMissedVisits();
    setFEVisits(response);
  }

  const getAllManagerMissedVisits = async () => {
    const response = await fetchAllManagerMissedVisits();
  ...
  const handleMarkComplete = async (visit) => {
    try {
      if(role =="fe"){
        await markFEVisitAsCompleted(visit.visitId);
      }else {
        await markManagerVisitAsCompleted(visit.visitId);
      }
      getAllFEMissedVisits();
      getAllManagerMissedVisits();
        ...
        {pendingVisits.length === 0 ? (
          ...
          <div className="grid gap-3">
            {pendingVisits.map((visit) => (
              <VisitRow key={visit.id} visit={visit} onMarkComplete={() => handleMarkComplete(visit)} />
            ))}
          </div>
        )}
      </div>      
...
const getUserName = (visit:any)=>{
  return  visit.userRole === "FIELD_EXECUTIVE" ? visit.feName : visit.managerName;
}

function VisitRow({ visit, onMarkComplete }: { visit: any; onMarkComplete?: () => void }) {
  return (
    <Card className="p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-medium text-sm">{getUserName(visit)}</span>
          <Badge variant="secondary" className="text-xs">{visit.employeeCode}</Badge>
        </div>
        <p className="text-sm text-muted-foreground">
          Visit to <span className="font-medium text-foreground">{visit.doctor.name}</span>
        </p>
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{visit.actualVisitTime}</span>
          <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{visit.location}</span>
        </div>
        <p className="text-xs text-destructive">Reason: {visit.notes}</p>
