...
export const fetchTodaysAndMissedVisits = async (managerId: number) => {
  const response = await API.get(`/manager-visits/today-scheduled-and-missed?managerId=${managerId}`, { 
    headers: {
        "Content-Type": "application/json", 
        "Accept": "application/json",
    },
...
export const fetchAllNextMonthVisits = async (managerId: number, week: number, day: number) => {
...
export const fetchAllCurrentMonthVisits = async (managerId: number, week: number, day: number) => {
  const response = await API.post(`/manager-visits/get-current-month-visits?managerId=${managerId}&weekNumber=${week}&dayOfWeek=${day}`, {
    headers: {
        "Content-Type": "application/json",
        "Accept": "application/json", 
    },
...
export const fetchManagerVisitReport = async (managerId: number, fromDate: any, toDate: any, status: string, category: string, docType: string) => {
  const response = await API.get(`/manager-visits/reports?managerId=${managerId}&from=${fromDate}&to=${toDate}&status=${status}&category=${category}&docType=${docType}`, {
    headers: { 
        "Content-Type": "application/json",
        "Accept": "application/json", 
    },
